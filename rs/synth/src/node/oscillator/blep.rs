...
pub struct BandLimitedSquareOsc {
    pub freq: f32,
    pub sr: usize,
    input_order: Vec<usize>,
    pub blep: BLEP,
...
impl std::default::Default for BandLimitedSquareOsc {
    fn default() -> Self {
        Self {
            freq: 1.0,
            sr: 44100,
...
impl<const N: usize> Node<N> for BandLimitedSquareOsc {
    fn process(&mut self, inputs: &mut HashMap<usize, Input<N>>, output: &mut [Buffer<N>]) {
        match inputs.len() {
            0 => {
                self.blep.set_freq(self.freq);
                for i in 0..N {
                    output[0][i] = self.blep.square();
                }
            },
            1 => {
                    let mod_input =  match self.input_order.len() {
...
pub struct BandLimitedTriangleOsc {
    pub freq: f32,
    pub sr: usize,
    input_order: Vec<usize>,
    pub blep: BLEP,
...
impl std::default::Default for BandLimitedTriangleOsc {
    fn default() -> Self {
        Self {
            freq: 1.0,
            sr: 44100,
...
impl<const N: usize> Node<N> for BandLimitedTriangleOsc {
    fn process(&mut self, inputs: &mut HashMap<usize, Input<N>>, output: &mut [Buffer<N>]) {
        match inputs.len() {
            0 => {
                self.blep.set_freq(self.freq);
                for i in 0..N {
                    output[0][i] = self.blep.triangle();
                }
            },
            1 => {
                    let mod_input =  match self.input_order.len() {
