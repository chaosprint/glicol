    ...
    fn send_msg(&mut self, info: Message) {
        ...
        match info {
            Message::SetToNumber(pos, value) => {
                match pos {
                    0 => {
                        self.context.graph[self.context.tags["env_amp"]].node.send_msg(Message::SetToNumber(0, value))
                    },
                    1 => {
                        self.context.graph[self.context.tags["env_amp"]].node.send_msg(Message::SetToNumber(1, value))
                    },
                    _ => {}
                }
            },
